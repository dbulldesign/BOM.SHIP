...
- **All releases / notes:** https://github.com/dbulldesign/BOM.SHIP/releases/latest

Download the HTML and double-click it — no install, no internet needed. When a
newer version is published, the app shows a "Download" banner that fetches the
latest file for you.
...
3. **Merge to `main`.** That's it — the **Build & Release** workflow
   (`.github/workflows/release.yml`) sees the new `APP_VERSION`, builds the
   offline file, and publishes it as a GitHub Release (tag `v1.1.0`) with the
   launcher bundle attached. Versions that are already released are skipped, so
   nothing is ever duplicated.

The in-app update check reads that release automatically, so existing users are
notified next time they're online.

Optional manual triggers (rarely needed): **Actions → Build & Release → Run
workflow** (type a tag), or `git tag v1.1.0 && git push origin v1.1.0`.
