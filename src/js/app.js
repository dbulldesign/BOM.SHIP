...
const COL_DEFAULTS = {qty:54,type:78,tag:118,mfr:120,part:130,desc:200,unitCost:88,mfrMult:62,markup:78,unitSell:88,extCost:96,extSell:104,source:130,notes:150,actions:92};
...
function sectionTable(kind, rows, defMarkup, label, tickClass){
  ...
  function fixtureRowHTML(r, i){
    ...
    const linkBtn = allowAccessories
      ...
      : '';
    return `<tr data-rk="${kind}" data-ri="${i}" class="${linkCls}${picking}">
      ...
      <td style="width:92px" class="no-print row-actions">
...
function allowanceFreightRows(kind){
  ...
  function line(label, pctField, pctVal, cost, sell, costOvField, sellOvField){
    ...
    return `<div class="af-line">
      ...
      <span class="af-cost">Cost <input class="af-num num${numOrNull(costOv)!=null?' af-ov':''}" inputmode="decimal" value="${cost2(cost)}" data-afov="${costOvField}" title="Type to override; clear to use the %"></span>
      <span class="af-sell">Sell <input class="af-num num sell${numOrNull(sellOv)!=null?' af-ov':''}" inputmode="decimal" value="${cost2(sell)}" data-afov="${sellOvField}" title="Type to override; clear to use the %"></span>
    </div>`;
  }
  if(kind==='fixtures'){
