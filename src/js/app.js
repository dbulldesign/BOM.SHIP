...
const COL_DEFAULTS = {qty:54,type:78,tag:118,mfr:120,part:130,desc:200,unitCost:88,mfrMult:62,markup:78,unitSell:88,extCost:96,extSell:104,source:130,notes:150,actions:74};
...
function sectionTable(kind, rows, defMarkup, label, tickClass){
  ...
  function fixtureRowHTML(r, i){
    ...
    const tagCell = allowAccessories
      ...
      : '';
    return `<tr data-rk="${kind}" data-ri="${i}">
...
/* Guard inserts/edits into an approved+locked base. Returns true if allowed,
   otherwise prompts to unlock and returns false. */
function guardBaseEdit(){
  if(baseEditAllowed()) return true;
  showConfirm('Edit approved base?', 'This option is approved and its base is locked to protect the agreed bid. Unlock the base to add parts? (Change orders are the usual way to add scope.)', 'Unlock base', 'primary').then(ok=>{
    if(ok){ _baseUnlockAck = true; state.options[state.current].baseLocked = false; markDirty(); render(); toast('Base unlocked'); }
  });
  return false;
}
