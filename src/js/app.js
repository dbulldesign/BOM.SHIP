...
function blankService(opt){
  ...
  return {id:uid(), qty:0, unit:opt.unit||"Days", location:(opt.location!==undefined?opt.location:"In Office"),
          desc:opt.desc||"", sellRate:(opt.sellRate!=null?opt.sellRate:0), source:(opt.source||""), note:"", addons:(opt.addons||[])};
}
...
function svcLine(o, addons){
  if(addons) o = {...o, addons: addons.map(a=>blankServiceAddon(a))};
...
function defaultServiceRows(type){
  ...
  /* Ninja / Design — unchanged for now (to be reworked from the template next). */
  return [
    ...
    blankService({unit:"Hours", location:"In Office", desc:"In office", sellRate:150}),
    blankService({unit:"Hours", location:"On Site",   desc:"On site",   sellRate:250}),
  ];
}
