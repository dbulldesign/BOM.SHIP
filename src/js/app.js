...
/* accessory presets pulled from the shipping template, with default prices and KLS est source.
   These are the built-in DEFAULTS — the user can customize the live list (add / edit / remove /
   reorder / reset) via the preset manager; their version is stored app-level in localStorage
   (NOT in the job .json) so it follows the computer, not the file. See loadPresets(). */
const ACCESSORY_PRESETS_DEFAULT = [
...
function blankService(opt){
  ...
  return {id:uid(), qty:0, unit:opt.unit||"Days", location:(opt.location!==undefined?opt.location:"In Office"),
          desc:opt.desc||"", sellRate:(opt.sellRate!=null?opt.sellRate:0), source:(opt.source||""), note:"",
          allowAddons:!!opt.allowAddons, addons:(opt.addons||[])};
}
...
/* Accessory-style add-on presets, by the unit they attach to (all priced as DK est).
   Built-in DEFAULTS — see ACCESSORY_PRESETS_DEFAULT note and loadPresets(). */
const TRIP_ADDONS_DEFAULT = [
...
/* ---- Configurable presets (app-level, persisted in localStorage, NOT in the job .json) ----
   Users can add / edit / remove / reorder the accessory and service add-on lists via the
   preset manager (openPresetManager). We keep the built-in *_DEFAULT arrays as the fallback
   and the "Reset to defaults" target. Each list is sanitized on load so a corrupt or
   partial saved value can never break the add menus. */
const PRESETS_KEY = 'lbom_presets_v1';
let _presets = null;
function _clonePresets(arr){ return JSON.parse(JSON.stringify(arr)); }
function sanitizeAccessoryPreset(p){
  p = p||{};
  return {label:String(p.label!=null?p.label:'').trim()||'Accessory', desc:String(p.desc||''),
          part:String(p.part||''), mfr:String(p.mfr||'MISC.'),
          unitCost:numOr(p.unitCost,0), source:String(p.source||'')};
}
function sanitizeAddonPreset(p){
  p = p||{};
  return {label:String(p.label!=null?p.label:'').trim()||'Add-on', desc:String(p.desc||''),
          sellRate:numOr(p.sellRate,0), source:String(p.source||'')};
}
function loadPresets(){
  if(_presets) return _presets;
  let saved = null;
  try{ saved = JSON.parse(localStorage.getItem(PRESETS_KEY)||'null'); }catch(e){}
  _presets = {
    accessories: (saved && Array.isArray(saved.accessories)) ? saved.accessories.map(sanitizeAccessoryPreset) : _clonePresets(ACCESSORY_PRESETS_DEFAULT),
    tripAddons:  (saved && Array.isArray(saved.tripAddons))  ? saved.tripAddons.map(sanitizeAddonPreset)      : _clonePresets(TRIP_ADDONS_DEFAULT),
    dayAddons:   (saved && Array.isArray(saved.dayAddons))   ? saved.dayAddons.map(sanitizeAddonPreset)       : _clonePresets(DAY_ADDONS_DEFAULT),
  };
  return _presets;
}
function savePresets(){ try{ localStorage.setItem(PRESETS_KEY, JSON.stringify(loadPresets())); }catch(e){} }
function resetPresets(){
  _presets = {
    accessories: _clonePresets(ACCESSORY_PRESETS_DEFAULT),
    tripAddons:  _clonePresets(TRIP_ADDONS_DEFAULT),
    dayAddons:   _clonePresets(DAY_ADDONS_DEFAULT),
  };
  savePresets();
}
function accessoryPresets(){ return loadPresets().accessories; }
function addonsForUnit(unit){
  const p = loadPresets();
  return unit==='Trips' ? p.tripAddons : (unit==='Days' ? p.dayAddons : null);
}
...
/* Default service rows per group type, matching the firm's BOM template
   (Supplier Services / Controls Services blocks). Location is taken from the
   In-Office / On-Site prefix and the prefix is dropped from the description,
   since the app shows location in its own column. The Travel and Site-Visit
   lines come pre-loaded with the standard add-ons (still editable via the
   + Add travel item / + Add day item menus). */
function svcLine(o, addons){
  /* Only Travel / Site-Visit lines (the ones we hand add-ons) get the
     "+ Add travel/day item" menu — allowAddons gates it. */
  if(addons) o = {...o, allowAddons:true, addons: addons.map(a=>blankServiceAddon(a))};
  return blankService(o);
}
const SVC_TRAVEL_ADDONS = [
  {desc:"RT FLIGHT",                 sellRate:500, source:"DK est"},
  {desc:"RT TRAVEL - NYC AIRPORT",   sellRate:75,  source:"DK est"},
  {desc:"RT TRAVEL - SITE AIRPORT",  sellRate:75,  source:"DK est"},
];
const SVC_SITEVISIT_ADDONS = [
  {desc:"HOTEL",                         sellRate:250, source:"DK est"},
  {desc:"CAR RENTAL / RT TRAVEL - SITE", sellRate:75,  source:"DK est"},
  {desc:"BREAKFAST",                     sellRate:10,  source:"DK est"},
  {desc:"LUNCH",                         sellRate:20,  source:"DK est"},
  {desc:"DINNER",                        sellRate:50,  source:"DK est"},
];
function defaultServiceRows(type){
  if(type==='supplier'){
    return [
      svcLine({unit:"Days", location:"In Office", desc:"COORDINATION OF FIXTURE INSTALLATION & WIRING. TECH SUPPORT TO INSTALLERS.", sellRate:1000, source:"KLS 2024"}),
      svcLine({unit:"Days", location:"In Office", desc:"CREATE FIXTURE SHOP DRAWINGS. REVIEW OF MANUFACTURER/MILLWORKER SHOPS, FIELD DIMENSIONS (BY OTHERS). LOW VOLTAGE TRANSFORMER/POWER SUPPLY LAYOUTS.", sellRate:1000, source:"KLS 2024"}),
      svcLine({unit:"Days", location:"In Office", desc:"", sellRate:1000, source:"KLS 2024"}),
      svcLine({unit:"Days", location:"On Site", desc:"KICK-OFF MEETING", sellRate:1500, source:"KLS 2024"}),
      svcLine({unit:"Days", location:"On Site", desc:"COORDINATION OF FIXTURE INSTALLATION & WIRING. TECH SUPPORT TO INSTALLERS.", sellRate:1500, source:"KLS 2024"}),
      svcLine({unit:"Days", location:"On Site", desc:"INSTALLATION MANAGEMENT/ASSISTANCE TO EC.", sellRate:1500, source:"KLS 2024"}),
      svcLine({unit:"Lump Sum", location:"—", desc:"SITE MANUAL & CLOSE-OUT DOCUMENTS", sellRate:375, source:"KLS 2024"}),
      svcLine({unit:"Trips", location:"—", desc:"TRAVEL EXPENSES TO AND FROM SITE (ROUND TRIP)", sellRate:0}, SVC_TRAVEL_ADDONS),
      svcLine({unit:"Days", location:"—", desc:"SITE VISIT EXPENSES (HOTEL, FOOD, TRANSPORT)", sellRate:0}, SVC_SITEVISIT_ADDONS),
    ];
  }
  if(type==='controls'){
    return [
      svcLine({unit:"Days", location:"In Office", desc:"PRESET DIMMING SYSTEM COORDINATION", sellRate:1000, source:"KLS 2024"}),
      svcLine({unit:"Days", location:"In Office", desc:"PRESET DIMMING SYSTEM PROGRAMMING AND CONFIGURATION", sellRate:1000, source:"KLS 2024"}),
      svcLine({unit:"Days", location:"In Office", desc:"COORDINATION OF EMERGENCY DIMMING REQUIREMENTS", sellRate:1000, source:"KLS 2024"}),
      svcLine({unit:"Days", location:"On Site", desc:"PRE-WIRE", sellRate:1500, source:"KLS 2024"}),
      svcLine({unit:"Days", location:"On Site", desc:"PROGRESS", sellRate:1500, source:"KLS 2024"}),
      svcLine({unit:"Days", location:"On Site", desc:"COMMISSIONING, PROGRAMMING & CLIENT TRAINING", sellRate:1500, source:"KLS 2024"}),
      svcLine({unit:"Trips", location:"—", desc:"TRAVEL EXPENSES TO AND FROM SITE (ROUND TRIP)", sellRate:0}, SVC_TRAVEL_ADDONS),
      svcLine({unit:"Days", location:"—", desc:"SITE VISIT EXPENSES (HOTEL, FOOD, TRANSPORT)", sellRate:0}, SVC_SITEVISIT_ADDONS),
    ];
  }
  /* Ninja / Design — Days then Hours (in-office / on-site), then the standard
     Travel and Site-Visit add-on lines. */
  return [
    svcLine({unit:"Days",  location:"In Office", desc:"", sellRate:1500}),
    svcLine({unit:"Days",  location:"On Site",   desc:"", sellRate:2500}),
    svcLine({unit:"Hours", location:"In Office", desc:"", sellRate:150}),
    svcLine({unit:"Hours", location:"On Site",   desc:"", sellRate:250}),
    svcLine({unit:"Trips", location:"—", desc:"TRAVEL EXPENSES TO AND FROM SITE (ROUND TRIP)", sellRate:0}, SVC_TRAVEL_ADDONS),
    svcLine({unit:"Days",  location:"—", desc:"SITE VISIT EXPENSES (HOTEL, FOOD, TRANSPORT)", sellRate:0}, SVC_SITEVISIT_ADDONS),
  ];
}
...
function blankOption(name){
  return { name:name, fixtureMarkup:50, controlMarkup:50, serviceMarkup:50,
           ...
           approved:false, approvedBy:"", approvedAt:"", revisions:[], changeOrders:[],
...
const AUTOSAVE_MAX = 30;            // keep a deeper timeline...
const AUTOSAVE_BYTES = 1500000;     // ...but cap total bytes so we don't blow the localStorage quota
...
function doAutosave(){
  try{
    ...
    /* byte cap: drop the oldest snapshots until the buffer fits */
    let str = JSON.stringify(arr);
    while(arr.length > 1 && str.length > AUTOSAVE_BYTES){ arr.pop(); str = JSON.stringify(arr); }
    localStorage.setItem(AUTOSAVE_KEY, str);
    updateSaveStamp();
  }catch(e){ /* localStorage full or unavailable — fail silently */ }
}
/* Lightweight per-snapshot stats for the timeline change-summary (rows + sell). */
function snapStats(json){
  try{
    const d = JSON.parse(json); let rows=0, sell=0;
    (d.options||[]).forEach(o=>{
      ['fixtures','controls'].forEach(k=> (o[k]||[]).forEach(r=>{ if(!r.isSection){ rows++; (r.accessories||[]).forEach(()=>rows++); } }));
      try{ sell += bomTotals(o).sub; }catch(e){}
    });
    return {rows, sell};
  }catch(e){ return {rows:0, sell:0}; }
}
function snapSummary(curJson, prevJson){
  const c = snapStats(curJson);
  if(!prevJson) return c.rows+' rows · '+money(c.sell);
  const p = snapStats(prevJson);
  const dr = c.rows - p.rows, ds = c.sell - p.sell, parts = [];
  if(dr) parts.push((dr>0?'+':'')+dr+' row'+(Math.abs(dr)===1?'':'s'));
  if(Math.abs(ds) >= 0.005) parts.push((ds>0?'+':'−')+money(Math.abs(ds)));
...
function showBackups(){
  ...
  if(arr.length===0){
    ...
    list.innerHTML = `<div class="backups-count">${arr.length} snapshot${arr.length===1?'':'s'} — newest first</div>` +
      arr.map((s,i)=>`
      <div class="backup-item">
        <div class="bi-info"><b>${esc(s.name)}</b><span class="bi-when">${fmtWhen(s.ts)}</span><span class="bi-sum">${esc(snapSummary(s.json, arr[i+1] && arr[i+1].json))}</span></div>
...
function sortedRows(rows, kind){
  ...
  }).sort((a,b)=> cmpSortVals(a.val, b.val, NUM.has(sc.col), sc.dir));
}
...
/* Reflect the live selection on each table's header checkbox: checked when every
   row is selected, indeterminate when only some are (so it can also clear them). */
function applySelAllBoxes(){
  const pane = document.getElementById('pane'); if(!pane) return;
  const o = state.options[state.current]; if(!o) return;
  pane.querySelectorAll('input[data-selall]').forEach(cb=>{
    const kind = cb.getAttribute('data-selall');
    const data = (o[kind]||[]).filter(r=>!r.isSection);
    const sel = data.filter(r=>bomSel.has(r.id)).length;
    cb.checked = data.length>0 && sel===data.length;
    cb.indeterminate = sel>0 && sel<data.length;
  });
}
...
/* ===== Custom Tag autocomplete =====================================================
   The native <datalist> on the row Tag cells gives inconsistent keyboard behaviour
   (arrows/Tab/Enter) and the suggestion list can linger. This small combobox replaces
   it: type to filter, ↑/↓ to highlight, Enter or Tab to accept the highlight, Esc or
   leaving the field to dismiss, and click to pick. One floating element is reused for
   every Tag input (the pane re-renders constantly, so the popup lives on <body>). */
const tagCombo = { el:null, input:null, items:[], hi:-1, suppress:false };
/* After an accept the pane re-renders and may restore focus to the same Tag cell —
   briefly suppress auto-opening so the popup doesn't spring back unwanted. */
function tagComboSuppress(){ tagCombo.suppress = true; setTimeout(()=>{ tagCombo.suppress = false; }, 250); }
function tagComboElem(){
  if(!tagCombo.el){
    const d = document.createElement('div');
    d.className = 'tag-combo'; d.hidden = true;
    d.addEventListener('mousedown', e=>{
      const b = e.target.closest('[data-tagopt]'); if(!b) return;
      e.preventDefault();                       // keep focus on the input; we commit explicitly
      tagComboAccept(b.getAttribute('data-tagopt'));
    });
...
function tagComboOpen(input){ tagCombo.input = input; tagComboFilter(); }
function tagComboFilter(){
  const input = tagCombo.input; if(!input) return;
  const q = (input.value||'').trim().toLowerCase();
  const all = tagOptions();
  tagCombo.items = q ? all.filter(t=>t.toLowerCase().includes(q)) : all.slice();
  tagCombo.hi = tagCombo.items.length ? 0 : -1;
  tagComboRender();
}
function tagComboRender(){
  const el = tagComboElem();
  if(!tagCombo.input || !tagCombo.items.length){ el.hidden = true; return; }
  el.innerHTML = tagCombo.items.map((t,idx)=>
    `<button type="button" data-tagopt="${esc(t)}"${idx===tagCombo.hi?' class="hi"':''}>${esc(t)}</button>`).join('');
  const r = tagCombo.input.getBoundingClientRect();
  el.style.left = (window.scrollX + r.left) + 'px';
  el.style.top  = (window.scrollY + r.bottom + 2) + 'px';
  el.style.minWidth = r.width + 'px';
  el.hidden = false;
  const hiBtn = el.querySelector('button.hi'); if(hiBtn) hiBtn.scrollIntoView({block:'nearest'});
}
function tagComboClose(){ tagCombo.input=null; tagCombo.items=[]; tagCombo.hi=-1; if(tagCombo.el) tagCombo.el.hidden = true; }
function tagComboMove(delta){
  if(!tagCombo.items.length) return;
  tagCombo.hi = Math.max(0, Math.min(tagCombo.items.length-1, tagCombo.hi + delta));
  tagComboRender();
}
function tagComboActive(el){ return tagCombo.input===el && tagCombo.el && !tagCombo.el.hidden && tagCombo.items.length; }
function tagComboAccept(val){
  const input = tagCombo.input; tagComboClose(); tagComboSuppress();
  if(!input) return;
  input.value = val;
  input.dispatchEvent(new Event('change', {bubbles:true}));   // commit through the normal path
}
...
function openSettings(){
  ...
  const body = `
    ...
    <div class="set-opt"><div style="width:100%">
      ...
      <div class="set-sub">Customize the accessory and service add-on lists (shared across all jobs on this computer).</div>
      <div class="lib-io" style="margin-top:6px"><button type="button" class="ghost" id="setEditPresets">Edit accessory &amp; service presets…</button></div>
    </div></div>`;
  openModal({
...
function searchAllOptions(q){
  ...
  if(!q) return _gsHits;
  /* option names */
  state.options.forEach((o,oi)=>{ if(String(o.name||'').toLowerCase().includes(q)) _gsHits.push({type:'option', oi, optName:o.name}); });
  /* fixtures & controls — match BOM text, then fall back to their ship-schedule data */
  state.options.forEach((o,oi)=>{
    ['fixtures','controls'].forEach(kind=>{
      (o[kind]||[]).forEach(r=>{
        if(r.isSection) return;
        const bomHay = [r.type,r.mfr,r.desc,r.part,r.tag,r.note].join(' ').toLowerCase();
        if(bomHay.includes(q)){ _gsHits.push({type:'bom', oi, optName:o.name, kind, r}); return; }
        const m = state.shipMeta[r.id] || {};
        const metaFields = {po:'PO', tracking:'Tracking', shipper:'Shipper', comments:'Comments', userStatus:'Status'};
        for(const k in metaFields){
          if(m[k] && String(m[k]).toLowerCase().includes(q)){
            _gsHits.push({type:'ship', oi, optName:o.name, kind, r, field:k, fieldLabel:metaFields[k], val:m[k]});
            break;
          }
        }
      });
...
function gsRun(hi){
  const h = _gsHits[hi]; if(!h) return; closeModal();
  if(h.type==='option'){ setView('estimate'); selectOption(h.oi); return; }
  if(h.type==='ship'){
    setView('shipping'); shipFilter = h.oi; shipFilterOpen = true;
    shipColFilters = { [h.field]: String(h.val) };
    renderShipping(); return;
  }
  /* bom */ setView('estimate'); selectOption(h.oi); bomFilter = (h.r.part||h.r.desc||h.r.type||''); render();
}
function openGlobalSearch(){
  const results = (q)=>{
    const hits = searchAllOptions(q);
    if(!q.trim()) return `<div class="lib-empty">Search every option's fixtures &amp; controls, the options themselves, and ship-schedule data (PO, tracking, shipper, status…).</div>`;
    if(!hits.length) return `<div class="lib-empty">No matches for "${esc(q)}".</div>`;
    return `<div class="lib-list">${hits.slice(0,300).map((h,i)=>{
      if(h.type==='option') return `<button class="gs-item" data-hi="${i}">
        <span class="gs-opt">${esc(h.optName)}</span><span class="lib-desc">Jump to this option</span><span class="gs-kind">Option</span></button>`;
      const tagTxt = h.type==='ship' ? `${esc(h.fieldLabel)}: ${esc(String(h.val))}` : `${esc(h.r.desc||'')}${h.r.part?' · '+esc(h.r.part):''}`;
      return `<button class="gs-item" data-hi="${i}">
        <span class="gs-opt">${esc(h.optName)}</span>
        <span class="lib-type">${esc(h.r.type||'—')}</span>
        <span class="lib-desc">${tagTxt}</span>
        <span class="gs-kind">${h.type==='ship'?'Schedule':(h.kind==='controls'?'Control':'Fixture')}</span>
      </button>`;
    }).join('')}</div>`;
  };
  const body = `<input class="lib-search" id="gsSearch" placeholder="Search options, fixtures, controls, PO, tracking, status…" autocomplete="off">
    <div id="gsWrap">${results('')}</div>`;
  openModal({
    title:'Search this job', bodyHTML:body, wide:true, cancelLabel:'Close',
    onOpen(back){
      const wrap = back.querySelector('#gsWrap'); const search = back.querySelector('#gsSearch');
      const bind = ()=> wrap.querySelectorAll('.gs-item').forEach(b=> b.addEventListener('click', ()=> gsRun(+b.dataset.hi)));
...
/* Shared column comparator: empty cells always sink to the bottom (whichever the
   sort direction); the rest sort numerically, or alphabetically with natural
   number ordering so F1 < F2 < F10 (not F1 < F10 < F2). */
function cmpSortVals(av, bv, isNum, dir){
  const aE = av===''||av==null, bE = bv===''||bv==null;
  if(aE && bE) return 0;
  if(aE) return 1;
  if(bE) return -1;
  if(isNum) return (numOr(av,0)-numOr(bv,0))*dir;
...
function sectionTable(kind, rows, defMarkup, label, tickClass){
  ...
  function fixtureRowHTML(r, i){
    ...
    const tagCell = allowAccessories
      ? `<td class="col-tag" data-label="Tag" style="width:118px"><input class="tag-input" autocomplete="off" value="${esc(r.tag||'')}" placeholder="Tag" ${da} data-f="tag"></td>`
      : '';
  ...
  function accessoryRowsHTML(r, i){
    ...
    const accHTML = accs.map((a,ai)=>{
      ...
      return `<tr class="acc-row">
        ...
        <td data-label="Type" style="width:78px"><span class="acc-pill">Part</span></td>
  ...
  return `<div class="section${catOpen?'':' cat-collapsed'}">
    ...
    <table class="bom-table">
      ...
      <thead><tr>
        <th class="col-lead no-print">${selCol?'<input type="checkbox" tabindex="-1" data-selall="'+kind+'" title="Select / clear all in this table" onclick="toggleSelectAll(\''+kind+'\', this.checked)">':''}</th>
        ${th('qty','Qty','r')}
        ${th('type','Type')}
...
function servicesTable(opt){
  ...
  const groupHTML = groups.map((g,gi)=>{
    ...
    const rowsHTML = (g.rows||[]).map((s,si)=>{
      ...
      /* Only Travel / Site-Visit lines (allowAddons) carry the add-on menu. */
      const allowAddons = !!s.allowAddons || (s.addons && s.addons.length>0);
      const presets = allowAddons ? addonsForUnit(s.unit) : null;
      let addonsHTML = '';
      if(presets){
  ...
  document.getElementById("pane").innerHTML = `
    ...
    <div class="opt-head ${opt.approved?'is-approved':''}">
      ...
      <div class="opt-tools no-print">
        ...
        ${approveBtn}
        <button class="ghost" onclick="openHistory()" title="Revision history & diffs for this option">🕘 History${ensureRevs(opt).length?' ('+ensureRevs(opt).length+')':''}</button>
...
/* ================= Revision history & diffs (per option) ================= */
const REV_MAX = 20;
const REV_FIELDS = [['qty','Qty'],['type','Type'],['mfr','Mfr'],['desc','Description'],['part','Part #'],['tag','Tag'],['unitCost','Unit cost'],['mfrMult','Mfr ×'],['markup','Markup'],['note','Note'],['source','Source']];
function ensureRevs(opt){ if(!Array.isArray(opt.revisions)) opt.revisions = []; return opt.revisions; }
/* A snapshot of the option's BOM data (excludes the revisions array itself). */
function optionSnapshot(opt){
  const keys = ['name','fixtureMarkup','controlMarkup','serviceMarkup','allowancePct','freightPct','controlAllowancePct','allowCostOv','allowSellOv','freightCostOv','freightSellOv','ctrlAllowCostOv','ctrlAllowSellOv','fixtures','controls','services','changeOrders'];
  const o = {}; keys.forEach(k=>{ if(opt[k]!==undefined) o[k]=opt[k]; });
  return JSON.parse(JSON.stringify(o));
}
function pushRevision(opt, label, by){
  const revs = ensureRevs(opt);
  revs.push({ id:uid(), ts:Date.now(), by:by||state.preparedBy||'', label:label||'Revision', snap:optionSnapshot(opt) });
  if(revs.length > REV_MAX) revs.splice(0, revs.length - REV_MAX);
  markDirty();
}
function saveRevisionNow(){
  const opt = state.options[state.current];
  pushRevision(opt, 'Manual revision', state.preparedBy);
  toast('Revision saved'); if(document.getElementById('revHistBody')) openHistory();
}
/* Flatten an option/snapshot's fixtures+controls into id-keyed rows for diffing. */
function revRows(snap){
  const map = new Map();
  ['fixtures','controls'].forEach(kind=> (snap[kind]||[]).forEach(r=>{ if(r && !r.isSection && r.id) map.set(r.id, {kind, row:r}); }));
  return map;
}
function diffSnaps(aSnap, bSnap){
  const A = revRows(aSnap), B = revRows(bSnap);
  const added=[], removed=[], changed=[];
  B.forEach((b,id)=>{ if(!A.has(id)) added.push(b); });
  A.forEach((a,id)=>{ if(!B.has(id)) removed.push(a); });
  A.forEach((a,id)=>{
    if(!B.has(id)) return; const b=B.get(id);
    const ch=[];
    REV_FIELDS.forEach(([f])=>{ const av=a.row[f]??'', bv=b.row[f]??''; if(String(av)!==String(bv)) ch.push({field:f, from:av, to:bv}); });
    if(ch.length) changed.push({kind:b.kind, row:b.row, changes:ch});
  });
  let subA=0, subB=0; try{ subA=bomTotals(aSnap).sub; }catch(e){} try{ subB=bomTotals(bSnap).sub; }catch(e){}
  return {added, removed, changed, subA, subB};
}
function restoreRevision(revId){
  const opt = state.options[state.current];
  const rev = ensureRevs(opt).find(r=>r.id===revId); if(!rev) return;
  showConfirm('Restore revision', `Replace this option's bill of materials with the “${esc(rev.label)}” revision from ${fmtWhen(rev.ts)}? (A revision of the current state is saved first.)`, 'Restore', 'primary').then(ok=>{
    if(!ok) return;
    pushRevision(opt, 'Before restore', state.preparedBy);   // safety snapshot
    const s = rev.snap;
    ['name','fixtureMarkup','controlMarkup','serviceMarkup','allowancePct','freightPct','controlAllowancePct','allowCostOv','allowSellOv','freightCostOv','freightSellOv','ctrlAllowCostOv','ctrlAllowSellOv','fixtures','controls','services','changeOrders'].forEach(k=>{
      if(s[k]!==undefined) opt[k] = JSON.parse(JSON.stringify(s[k]));
    });
    markDirty(); closeModal(); render(); toast('Revision restored');
  });
}
function openHistory(){
  const opt = state.options[state.current];
  const revs = ensureRevs(opt).slice().reverse();   // newest first
  const entries = [{id:'__current', label:'Current (working copy)', ts:null, by:'', snap:optionSnapshot(opt)}].concat(revs);
  const optOpts = entries.map((e,i)=>`<option value="${e.id}">${e.ts?esc(fmtWhen(e.ts)):'Now'} — ${esc(e.label)}${e.by?(' · '+esc(e.by)):''}</option>`).join('');
  const aSel = entries[1] ? entries[1].id : '__current';   // default A = latest revision
  const body = `
    <p class="paste-help">Revisions are captured automatically when you approve an option, plus any you save manually. Compare two points or restore one. ${opt.approvedBy?`<b>Approved by ${esc(opt.approvedBy)}${opt.approvedAt?(' on '+esc(opt.approvedAt)):''}.</b>`:''}</p>
    <div class="rev-compare">
      <label>Compare <select id="revA">${optOpts.replace('value="'+aSel+'"','value="'+aSel+'" selected')}</select></label>
      <label>with <select id="revB"><option value="__current" selected>Now — Current (working copy)</option>${revs.map(e=>`<option value="${e.id}">${esc(fmtWhen(e.ts))} — ${esc(e.label)}</option>`).join('')}</select></label>
      <button class="ghost" id="revSaveNow">+ Save revision now</button>
    </div>
    <div id="revDiff" class="rev-diff"></div>
    <div class="rev-list">${revs.length?revs.map(r=>`<div class="rev-row"><span class="rev-when">${esc(fmtWhen(r.ts))}</span><span class="rev-label">${esc(r.label)}${r.by?(' · '+esc(r.by)):''}</span><button class="ghost" onclick="restoreRevision('${r.id}')">Restore</button></div>`).join(''):'<div class="dim">No saved revisions yet.</div>'}</div>`;
  openModal({ title:'Option history', wide:true, cancelLabel:'Close', bodyHTML:`<div id="revHistBody">${body}</div>`,
    onOpen(back){
      const snapOf = id => id==='__current' ? optionSnapshot(opt) : (ensureRevs(opt).find(r=>r.id===id)||{}).snap;
      const draw = ()=>{
        const a = back.querySelector('#revA').value, b = back.querySelector('#revB').value;
        const sa = snapOf(a), sb = snapOf(b);
        if(!sa || !sb){ back.querySelector('#revDiff').innerHTML=''; return; }
        const d = diffSnaps(sa, sb);
        const rowLine = (cls,sign,x)=>`<tr class="${cls}"><td>${sign}</td><td>${esc(x.kind)}</td><td><b>${esc(x.row.type||'—')}</b> ${esc(x.row.desc||x.row.part||'')}</td><td>${x.changes?x.changes.map(c=>`${esc(revFieldLabel(c.field))}: ${esc(String(c.from))} → ${esc(String(c.to))}`).join('<br>'):''}</td></tr>`;
        const rows = d.added.map(x=>rowLine('rev-add','＋',x)).join('') + d.removed.map(x=>rowLine('rev-rem','－',x)).join('') + d.changed.map(x=>rowLine('rev-chg','~',x)).join('');
        const ds = d.subB - d.subA;
        back.querySelector('#revDiff').innerHTML =
          `<div class="rev-summary">${d.added.length} added · ${d.removed.length} removed · ${d.changed.length} changed · subtotal ${money(d.subA)} → ${money(d.subB)} <b class="${ds>=0?'up':'down'}">(${ds>=0?'+':'−'}${money(Math.abs(ds))})</b></div>` +
          (rows? `<table class="rev-table"><thead><tr><th></th><th>Cat.</th><th>Item</th><th>Change</th></tr></thead><tbody>${rows}</tbody></table>` : '<div class="dim">No differences between these two.</div>');
      };
      back.querySelector('#revA').addEventListener('change', draw);
      back.querySelector('#revB').addEventListener('change', draw);
      back.querySelector('#revSaveNow').addEventListener('click', saveRevisionNow);
      draw();
    }
  });
}
function revFieldLabel(f){ const e=REV_FIELDS.find(x=>x[0]===f); return e?e[1]:f; }

function approveOption(){
  const o=state.options[state.current];
  o.approved = true; o.approvedBy = state.preparedBy||o.approvedBy||''; o.approvedAt = new Date().toLocaleDateString();
  pushRevision(o, 'Approved', state.preparedBy);
  markDirty(); render(); toast('Option approved & locked — revision saved; delete still works, add scope via a Change Order');
}
...
function bindPane(){
  ...
  if(!_paneDelegated){
    ...
    pane.addEventListener("focusin", e=>{ const t=e.target; if(t && 'value' in t) t._focusVal = t.value;
      if(t && t.classList && t.classList.contains('tag-input') && !tagCombo.suppress) tagComboOpen(t); });
    pane.addEventListener("input", e=>{ const t=e.target; if(t && t.classList && t.classList.contains('tag-input')) tagComboFilter(); });
    pane.addEventListener("focusout", e=>{ const t=e.target; if(t && t.classList && t.classList.contains('tag-input')) setTimeout(()=>{ if(document.activeElement!==t) tagComboClose(); }, 120); });
...
function onPaneKeydown(e){
  ...
  /* Tag combobox takes priority over the spreadsheet-style nav while it's open */
  if(el.classList && el.classList.contains('tag-input') && tagComboActive(el)){
    if(e.key==="ArrowDown"){ e.preventDefault(); tagComboMove(1); return; }
    if(e.key==="ArrowUp"){ e.preventDefault(); tagComboMove(-1); return; }
    if(e.key==="Escape"){ e.preventDefault(); tagComboClose(); return; }
    /* Enter / Tab: accept the highlight into the field, then fall through to the
       normal Enter (commit + add/move row) / Tab (commit + move column) handling. */
    if((e.key==="Enter" || e.key==="Tab") && tagCombo.hi>=0){ el.value = tagCombo.items[tagCombo.hi]; tagComboClose(); tagComboSuppress(); }
  }
  if(e.key==="Enter"){
...
function addAccessory(kind, i, presetIdx){
  ...
  row.accessories.push(blankAccessory(accessoryPresets()[presetIdx]));
...
function dupOption(){
  ...
  copy.name += " (copy)";
  copy.revisions = []; copy.approved = false; copy.approvedBy = ""; copy.approvedAt = "";   // fresh history for the copy
...
/* User-editable manual status list (app/browser-local — not in the job .json).
   Each: {name, printHidden}. The per-row chosen value lives in meta.userStatus. */
const SHIP_STATUS_KEY = 'lbom_ship_statuses_v1';
const DEFAULT_SHIP_STATUSES = ['Ordered','In Production','Shipped','In Stock','On Hold','Delivered','Cancelled'];
let shipStatuses = loadShipStatuses();
function loadShipStatuses(){
  try{
    const v = JSON.parse(localStorage.getItem(SHIP_STATUS_KEY));
    if(Array.isArray(v) && v.length) return v.map(s=> typeof s==='string'?{name:s,printHidden:false}:{name:String(s.name||''),printHidden:!!s.printHidden}).filter(s=>s.name);
  }catch(e){}
  return DEFAULT_SHIP_STATUSES.map(n=>({name:n, printHidden:false}));
}
function saveShipStatuses(){ try{ localStorage.setItem(SHIP_STATUS_KEY, JSON.stringify(shipStatuses)); }catch(e){} }
function printHiddenStatusSet(){ return new Set(shipStatuses.filter(s=>s.printHidden).map(s=>s.name)); }
function openStatusManager(){
  const draw = ()=>{
    const list = shipStatuses.map((s,i)=>`
      <div class="stm-row">
        <input class="stm-name" value="${esc(s.name)}" data-i="${i}" placeholder="Status name" aria-label="Status name">
        <label class="stm-ph"><input type="checkbox" class="stm-hide" data-i="${i}" ${s.printHidden?'checked':''}> hide from print</label>
        <button class="stm-mini" data-mv="up" data-i="${i}" ${i===0?'disabled':''} title="Move up">▲</button>
        <button class="stm-mini" data-mv="down" data-i="${i}" ${i===shipStatuses.length-1?'disabled':''} title="Move down">▼</button>
        <button class="stm-mini stm-del" data-del="${i}" title="Delete">✕</button>
      </div>`).join('');
    return `<p class="paste-help">Your shipping status options (shared across all jobs on this computer). “Hide from print” drops rows with that status from the printed schedule.</p>
      <div class="stm-list">${list||'<div class="dim">No statuses yet.</div>'}</div>
      <div class="lib-io"><button class="ghost" id="stmAdd">+ Add status</button></div>`;
  };
  openModal({
    title:'Edit ship statuses', wide:true, cancelLabel:'Done', bodyHTML:draw(),
    onOpen(back){
      const body = back.querySelector('.modal-body');
      const refresh = ()=>{ body.innerHTML = draw(); bind(); };
      function bind(){
        back.querySelectorAll('.stm-name').forEach(inp=> inp.addEventListener('change', ()=>{ shipStatuses[+inp.dataset.i].name = inp.value.trim()||'Status'; saveShipStatuses(); }));
        back.querySelectorAll('.stm-hide').forEach(cb=> cb.addEventListener('change', ()=>{ shipStatuses[+cb.dataset.i].printHidden = cb.checked; saveShipStatuses(); }));
        back.querySelectorAll('[data-mv]').forEach(b=> b.addEventListener('click', ()=>{ const i=+b.dataset.i, j=b.dataset.mv==='up'?i-1:i+1; if(j<0||j>=shipStatuses.length) return; const t=shipStatuses[i]; shipStatuses[i]=shipStatuses[j]; shipStatuses[j]=t; saveShipStatuses(); refresh(); }));
        back.querySelectorAll('[data-del]').forEach(b=> b.addEventListener('click', ()=>{ shipStatuses.splice(+b.dataset.del,1); saveShipStatuses(); refresh(); }));
        const add = back.querySelector('#stmAdd'); if(add) add.addEventListener('click', ()=>{ shipStatuses.push({name:'New status', printHidden:false}); saveShipStatuses(); refresh(); });
      }
      bind();
    },
    onCancel(){ renderShipping(); }   // refresh the dropdowns after editing the list
  });
}

/* ---- Preset manager: edit the accessory & service add-on quick-add lists ----
   App-level config (localStorage, shared across jobs on this computer; never written to a
   job .json). Add / edit / remove / reorder each list, or reset everything to the built-ins. */
function openPresetManager(){
  const P = loadPresets();
  const accCols = [
    {f:'label',   ph:'Menu label', w:'1.2fr'},
    {f:'desc',    ph:'Description', w:'1.4fr'},
    {f:'part',    ph:'Part No.',    w:'0.9fr'},
    {f:'mfr',     ph:'Mfr',         w:'0.8fr'},
    {f:'unitCost',ph:'Unit cost',   w:'0.7fr', num:true},
    {f:'source',  ph:'Price source',w:'0.9fr'},
  ];
  const addCols = [
    {f:'label',   ph:'Menu label', w:'1.2fr'},
    {f:'desc',    ph:'Description', w:'1.6fr'},
    {f:'sellRate',ph:'Sell rate',  w:'0.7fr', num:true},
    {f:'source',  ph:'Source',     w:'0.9fr'},
  ];
  const lists = [
    {key:'accessories', title:'Fixture / control accessories', sub:'Shown in the “+ Add accessory” menu on fixture and control rows.', cols:accCols, blank:{label:'New accessory',desc:'',part:'',mfr:'MISC.',unitCost:0,source:''}},
    {key:'tripAddons',  title:'Trip add-ons (Services · Trips)', sub:'Shown in the “+ Add travel item” menu on Services lines whose unit is Trips.', cols:addCols, blank:{label:'New trip item',desc:'',sellRate:0,source:''}},
    {key:'dayAddons',   title:'Day add-ons (Services · Days)',   sub:'Shown in the “+ Add day item” menu on Services lines whose unit is Days.',  cols:addCols, blank:{label:'New day item',desc:'',sellRate:0,source:''}},
  ];
  const drawList = (L)=>{
    const arr = P[L.key];
    const head = `<div class="pm-head" style="grid-template-columns:${L.cols.map(c=>c.w).join(' ')} auto">`
      + L.cols.map(c=>`<span>${esc(c.ph)}</span>`).join('') + `<span></span></div>`;
    const rows = arr.map((p,i)=>`
      <div class="pm-row" style="grid-template-columns:${L.cols.map(c=>c.w).join(' ')} auto">
        ${L.cols.map(c=>`<input class="${c.num?'num':''}" ${c.num?'inputmode="decimal"':''} value="${esc(c.num?String(numOr(p[c.f],0)):(p[c.f]||''))}" placeholder="${esc(c.ph)}" data-pl="${L.key}" data-i="${i}" data-f="${c.f}" aria-label="${esc(c.ph)}">`).join('')}
        <span class="pm-acts">
          <button class="stm-mini" data-pl="${L.key}" data-mv="up" data-i="${i}" ${i===0?'disabled':''} title="Move up">▲</button>
          <button class="stm-mini" data-pl="${L.key}" data-mv="down" data-i="${i}" ${i===arr.length-1?'disabled':''} title="Move down">▼</button>
          <button class="stm-mini stm-del" data-pl="${L.key}" data-del="${i}" title="Delete">✕</button>
        </span>
      </div>`).join('');
    return `<div class="pm-block">
      <div class="pm-title"><b>${esc(L.title)}</b><span class="set-sub">${esc(L.sub)}</span></div>
      ${head}
      <div class="pm-rows">${rows||'<div class="dim" style="padding:6px">No presets — add one below.</div>'}</div>
      <div class="lib-io"><button class="ghost" data-pl-add="${L.key}">+ Add preset</button></div>
    </div>`;
  };
  const draw = ()=>`<p class="paste-help">Quick-add presets for accessories and service add-ons. These live on <b>this computer</b> (shared across all jobs) and are never saved into a project file. Reorder with ▲▼; the order here is the order they appear in the add menus.</p>`
    + lists.map(drawList).join('')
    + `<div class="lib-io" style="margin-top:10px"><button class="ghost" id="pmReset">↺ Reset all to defaults</button></div>`;
  openModal({
    title:'Edit accessory & service presets', wide:true, cancelLabel:'Done', bodyHTML:draw(),
    onOpen(back){
      const body = back.querySelector('.modal-body');
      const refresh = ()=>{ body.innerHTML = draw(); bind(); };
      function bind(){
        back.querySelectorAll('input[data-pl]').forEach(inp=> inp.addEventListener('change', ()=>{
          const arr = P[inp.dataset.pl]; const row = arr[+inp.dataset.i]; if(!row) return;
          const f = inp.dataset.f;
          if(f==='unitCost' || f==='sellRate') row[f] = numOr(inp.value,0);
          else row[f] = inp.value;
          savePresets();
        }));
        back.querySelectorAll('button[data-mv]').forEach(b=> b.addEventListener('click', ()=>{
          const arr = P[b.dataset.pl]; const i=+b.dataset.i, j=b.dataset.mv==='up'?i-1:i+1;
          if(j<0||j>=arr.length) return; const t=arr[i]; arr[i]=arr[j]; arr[j]=t; savePresets(); refresh();
        }));
        back.querySelectorAll('button[data-del]').forEach(b=> b.addEventListener('click', ()=>{
          P[b.dataset.pl].splice(+b.dataset.del,1); savePresets(); refresh();
        }));
        back.querySelectorAll('button[data-pl-add]').forEach(b=> b.addEventListener('click', ()=>{
          const L = lists.find(x=>x.key===b.dataset.plAdd);
          P[L.key].push(JSON.parse(JSON.stringify(L.blank))); savePresets(); refresh();
        }));
        const reset = back.querySelector('#pmReset');
        if(reset) reset.addEventListener('click', async ()=>{
          const ok = await showConfirm('Reset presets?', 'Replace the accessory and service add-on lists with the built-in defaults? This only affects this computer.', 'Reset', 'danger');
          if(!ok) return;
          resetPresets();
          // refresh our local handle to the new arrays
          const np = loadPresets(); P.accessories=np.accessories; P.tripAddons=np.tripAddons; P.dayAddons=np.dayAddons;
          refresh(); toast('Presets reset to defaults');
        });
      }
      bind();
    },
    onCancel(){ render(); }   // re-render so any open add menus rebuild from the new lists
  });
}
function userStatusCell(l, editable){
  if(!editable) return '<td class="ustatus-cell"></td>';
  const cur = (l.meta && l.meta.userStatus) || '';
  const inList = !cur || shipStatuses.some(s=>s.name===cur);
  const opts = `<option value="">—</option>`
    + shipStatuses.map(s=>`<option ${cur===s.name?'selected':''}>${esc(s.name)}</option>`).join('')
    + (inList?'':`<option selected>${esc(cur)}</option>`);   // keep an unknown saved value visible
  return `<td class="ustatus-cell"><select onchange="setMetaField('${l.id}','userStatus',this.value)" title="Status">${opts}</select></td>`;
}
...
function blankMeta(){
  return { po:'', orderDate:'', recvDate:'', estShip:'', shipDate:'', shipper:'',
           tracking:'', delivery:'', comments:'', userStatus:'', invoiceId:null };
}
...
function renderShipping(){
  ...
  const filterBar = `
    <div class="ship-toolbar">
      <div class="ship-import">
        ...
        <button class="ghost" onclick="openStatusManager()" title="Edit the Status dropdown list">⚙ Statuses</button>
        <span class="ship-live-note">Live from the bill of materials.</span>
      </div>
      <div class="grow"></div>
      <div class="ship-export-group">
        <button class="ghost" onclick="openPurchaseOrders()" title="Group items by vendor and export one purchase order per manufacturer">🧾 Purchase orders</button>
  ...
  if(shipSort.col && !hasGroups){
    ...
    lines = lines.slice().sort((a,b)=>{
      ...
      else if(shipSort.col==='userStatus'){ av=(a.meta&&a.meta.userStatus)||''; bv=(b.meta&&b.meta.userStatus)||''; }
  ...
  const COLSPAN = 12 + SHIP_FIELDS.length + 1;   // +1 for the manual Status column
  const body = lines.map(l=>{
    ...
    const descCell = isAcc
      ...
      : `<td style="min-width:180px" class="mirror">${esc(l.desc)||'<span class="dim">—</span>'}</td>`;
    const catCell = isAcc
      ? `<td style="min-width:70px"><span class="acc-pill">Part</span></td>`
      ...
      <div class="ship-scroll">
        <table class="ship-table">
          <thead><tr>
            ...
            ${sth('sold','Sold $','r')}
            ${sth('status','Progress')}
            ${sth('userStatus','Status')}
            ${SHIP_FIELDS.map(f=>sth(f.k,f.label)).join('')}
            <th>Invoice</th>
          </tr>
          ${shipFilterOpen ? `<tr class="col-filter-row no-print">
            <th></th>${['optName','kind','type','mfr','desc','part'].map(scfIn).join('')}<th></th><th></th>${scfIn('status')}${scfIn('userStatus')}
...
const SHIP_PRINT_COLS = [
  ...
  {key:'qty',label:'Qty'},{key:'sold',label:'Sold $'},{key:'status',label:'Progress'},{key:'userStatus',label:'Status'},
...
/* Roll BOM items up by manufacturer (vendor). Costs are your cost (what you pay
   the vendor), not sell. Scope: 'current' option or 'all' options. */
function vendorGroups(scope){
  const opts = scope==='all' ? state.options : [state.options[state.current]];
  const map = new Map();
  const add = (mfr,qty,type,part,desc,unit,ext,optName)=>{
    const key = (String(mfr||'').trim()) || '(no manufacturer)';
    if(!map.has(key)) map.set(key, {mfr:key, items:[], totalCost:0, qty:0});
    const g = map.get(key); g.items.push({qty,type,part,desc,unit,ext,optName}); g.totalCost += ext; g.qty += numOr(qty,0);
  };
  opts.forEach(o=>{
    if(!o) return;
    [['fixtures',o.fixtureMarkup],['controls',o.controlMarkup]].forEach(([kind,dm])=>{
      (o[kind]||[]).forEach(r=>{
        if(r.isSection) return;
        const c = rowCalc(r,dm), eu = numOr(r.unitCost,0)*mfrMultOf(r);
        add(r.mfr, r.qty, r.type, r.part, r.desc, eu, c.extCost, o.name);
        (r.accessories||[]).forEach(a=>{ const ac=accCalc(a,r,dm), aeu=numOr(a.unitCost,0)*mfrMultOf(a); add(a.mfr||r.mfr, ac.qty, 'Acc', a.part, a.desc, aeu, ac.extCost, o.name); });
      });
    });
  });
  return [...map.values()].sort((a,b)=> a.mfr.localeCompare(b.mfr));
}
function exportPOsPDF(scope){
  const groups = vendorGroups(scope);
  if(!groups.length){ toast('No items to build POs from'); return; }
  const jsPDF = ensureJsPDF(); if(!jsPDF){ toast('PDF engine could not load'); return; }
  const M = 40;
  const doc = new jsPDF({unit:'pt', format:'letter', orientation:'portrait'});
  const pageW = doc.internal.pageSize.getWidth();
  const cols = [
    {key:'qty',label:'QTY',w:34,align:'right'},{key:'type',label:'TYPE',w:50},{key:'part',label:'PART NUMBER',w:120},
    {key:'desc',label:'DESCRIPTION',w:0},{key:'unit',label:'UNIT COST',w:74,align:'right'},{key:'ext',label:'EXT. COST',w:84,align:'right'}
  ];
  const fixed = cols.reduce((s,c)=>s+c.w,0); cols.find(c=>c.key==='desc').w = (pageW-2*M) - fixed;
  groups.forEach((g,gi)=>{
    if(gi>0) doc.addPage();
    let y = M;
    if(LOGO_DATA_URI && !LOGO_DATA_URI.startsWith('__')){ try{ doc.addImage(LOGO_DATA_URI,'PNG',M,y,84,26); }catch(e){} }
    doc.setFont('helvetica','bold'); doc.setTextColor.apply(doc,PDF_INK); doc.setFontSize(14);
    doc.text('PURCHASE ORDER', pageW-M, y+12, {align:'right'});
    doc.setFontSize(9); doc.setFont('helvetica','normal'); doc.setTextColor.apply(doc,PDF_MUTED);
    doc.text((state.company||'Focus Lighting'), pageW-M, y+26, {align:'right'});
    y += 38;
    doc.setTextColor.apply(doc,PDF_INK); doc.setFont('helvetica','bold'); doc.setFontSize(12);
    doc.text('Vendor: '+esc2pdf(g.mfr), M, y); y += 16;
    doc.setFont('helvetica','normal'); doc.setFontSize(9); doc.setTextColor.apply(doc,PDF_MUTED);
    const meta=[]; if(state.name) meta.push('Project: '+state.name); if(state.jobCode) meta.push('Job: '+state.jobCode); meta.push('Date: '+new Date().toLocaleDateString());
    doc.text(meta.join('     '), M, y); y += 14; doc.setTextColor.apply(doc,PDF_INK);
    const rows = g.items.map(it=>({kind:'data', cells:{qty:it.qty, type:it.type, part:it.part||'', desc:it.desc||'', unit:money(it.unit), ext:money(it.ext)}}));
    y = pdfTable(doc, cols, rows, y, M);
    y += 6; doc.setFont('helvetica','bold'); doc.setFontSize(10);
    doc.text('PO total', M+ (pageW-2*M) - 84 - 4, y+9, {align:'right'});
    doc.text(money(g.totalCost), M+(pageW-2*M), y+9, {align:'right'}); doc.setFont('helvetica','normal');
  });
  pdfPageNumbers(doc);
  doc.save((state.name? state.name.replace(/[^\w\- ]+/g,'').trim().replace(/\s+/g,'_'):'project')+'_purchase_orders.pdf');
  toast(groups.length+' purchase order'+(groups.length===1?'':'s')+' exported');
}
function exportPOsCSV(scope){
  const groups = vendorGroups(scope);
  if(!groups.length){ toast('No items to build POs from'); return; }
  const q = s => '"'+String(s==null?'':s).replace(/"/g,'""')+'"';
  const lines = [['Vendor','Option','Qty','Type','Part #','Description','Unit cost','Ext. cost'].join(',')];
  groups.forEach(g=> g.items.forEach(it=> lines.push([q(g.mfr),q(it.optName),it.qty,q(it.type),q(it.part),q(it.desc),numOr(it.unit,0).toFixed(2),numOr(it.ext,0).toFixed(2)].join(','))));
  const blob = new Blob([lines.join('\r\n')], {type:'text/csv'});
  const a = document.createElement('a'); a.href=URL.createObjectURL(blob); a.download=(state.name||'project')+'_purchase_orders.csv'; a.click(); URL.revokeObjectURL(a.href);
  toast('Purchase orders CSV exported');
}
let _poScope = 'current';
function openPurchaseOrders(){
  const draw = ()=>{
    const groups = vendorGroups(_poScope);
    const grand = groups.reduce((a,g)=>a+g.totalCost,0);
    const list = groups.length ? groups.map(g=>`<div class="po-row"><span class="po-vendor">${esc(g.mfr)}</span><span class="po-meta">${g.items.length} line${g.items.length===1?'':'s'} · ${g.qty.toLocaleString()} qty</span><span class="po-total">${money(g.totalCost)}</span></div>`).join('')
      : '<div class="lib-empty">No items in this scope.</div>';
    return `<p class="paste-help">Roll items up by manufacturer into purchase orders (at <b>your cost</b>). Export one PO per vendor.</p>
      <div class="po-scope"><label>Scope <select id="poScope">
        <option value="current" ${_poScope==='current'?'selected':''}>Current option (${esc(state.options[state.current]?.name||'')})</option>
        <option value="all" ${_poScope==='all'?'selected':''}>All options</option>
      </select></label><span class="po-grand">${groups.length} vendor${groups.length===1?'':'s'} · ${money(grand)}</span></div>
      <div class="po-list">${list}</div>
      <div class="lib-io"><button class="primary" id="poPdf">⬇ Export POs (PDF, one per vendor)</button><button class="ghost" id="poCsv">⬇ Export POs (CSV)</button></div>`;
  };
  openModal({ title:'Purchase orders by vendor', wide:true, cancelLabel:'Close', bodyHTML:draw(),
    onOpen(back){
      const body = back.querySelector('.modal-body');
      const bind = ()=>{
        const sc = back.querySelector('#poScope'); if(sc) sc.addEventListener('change', ()=>{ _poScope=sc.value; body.innerHTML=draw(); bind(); });
        const pdf = back.querySelector('#poPdf'); if(pdf) pdf.addEventListener('click', ()=>exportPOsPDF(_poScope));
        const csv = back.querySelector('#poCsv'); if(csv) csv.addEventListener('click', ()=>exportPOsCSV(_poScope));
      };
      bind();
    }
  });
...
function printShipSchedule(){
  let rows = scheduleForExport();
  /* exclude rows whose manual status is marked "hide from print" */
  const phs = printHiddenStatusSet();
  if(phs.size){
    rows = rows.filter(l=> (l.isCategory||l.isGroup) ? true : !phs.has((l.meta&&l.meta.userStatus)||''));
    rows = pruneEmptyShipBands(rows);
  }
  ...
  const bodyRows = rows.map(l=>{
    ...
    const cell = {
      ...
      status:{h:st}, userStatus:{h:esc2(m.userStatus)}, po:{h:esc2(m.po)}, orderDate:{h:esc2(m.orderDate)}, recvDate:{h:esc2(m.recvDate)},
...
/* ---- Multi-user conflict detection (File System Access path only) ----
   Record the file's lastModified+size at open and after each save; if the file
   on disk is newer than what we recorded when we try to save (or when the window
   regains focus), warn before overwriting. The download/upload fallback has no
   handle, so detection simply doesn't run there. */
let fileMeta = null;            // {lastModified, size} of the bound file as we last knew it
let _conflictBannerShown = false;
function recordFileMeta(file){ if(file) fileMeta = {lastModified:file.lastModified, size:file.size}; }
function fileIsNewer(file){ return fileMeta && file && file.lastModified > fileMeta.lastModified + 1; }
function chooseConflict(name){
  return new Promise(resolve=>{
    const back = document.createElement('div'); back.className='modal-backdrop'; back.style.display='flex';
    back.innerHTML = `<div class="modal modal-wide" onclick="event.stopPropagation()">
      <h4>This file changed on disk</h4>
      <p>“${esc(name)}” was modified by something else (another user or program) since you opened it. How do you want to proceed? <b>Reload</b> discards your current unsaved edits.</p>
      <div class="modal-btns conflict-btns">
        <button class="ghost" data-k="reload">Reload disk version</button>
        <button class="ghost" data-k="copy">Save as copy…</button>
        <button class="danger" data-k="overwrite">Overwrite</button>
        <button class="ghost" data-k="cancel">Cancel</button>
      </div></div>`;
    const done = k => { back.remove(); resolve(k); };
    back.addEventListener('click', e=>{ if(e.target===back) done('cancel'); });
    back.querySelectorAll('[data-k]').forEach(b=> b.addEventListener('click', ()=> done(b.dataset.k)));
    document.body.appendChild(back);
  });
}
async function checkFileConflictOnFocus(){
  if(!fileHandle || !fileMeta || _conflictBannerShown) return;
  try{ const cur = await fileHandle.getFile(); if(fileIsNewer(cur)){ _conflictBannerShown = true; showConflictBanner(fileHandle.name); } }catch(e){}
}
function showConflictBanner(name){
  if(document.getElementById('conflictBanner')) return;
  const b = document.createElement('div'); b.id='conflictBanner'; b.className='update-banner conflict-banner no-print';
  b.innerHTML = `<span class="ub-msg">⚠ “${esc(name)}” changed on disk (another user or program). Your unsaved edits are still here.</span>
    <span class="ub-actions"><button class="ub-update" onclick="reloadFromDisk()">Reload disk version</button><button class="ub-later" onclick="dismissConflictBanner()">Dismiss</button></span>`;
  document.body.appendChild(b); document.body.classList.add('has-update-banner');
}
function dismissConflictBanner(){ const b=document.getElementById('conflictBanner'); if(b) b.remove(); document.body.classList.remove('has-update-banner'); _conflictBannerShown=false; }
async function reloadFromDisk(){
  if(!fileHandle) return;
  try{ const cur = await fileHandle.getFile(); recordFileMeta(cur); dismissConflictBanner(); loadFromText(await cur.text(), fileHandle.name); }
  catch(e){ toast('Could not reload from disk'); }
}
  ...
  if(window.showSaveFilePicker){
    try{
      if(saveAs || !fileHandle){
        ...
        if(p!=='granted'){ fileHandle = await window.showSaveFilePicker({suggestedName:suggestedName(), types:FILE_TYPES}); fileMeta = null; }
      }
      /* conflict check: did the file change underneath us since we opened/saved it? */
      if(!saveAs && fileMeta){
        try{
          const cur = await fileHandle.getFile();
          if(fileIsNewer(cur)){
            const choice = await chooseConflict(fileHandle.name);
            if(choice==='cancel') return;
            if(choice==='reload'){ recordFileMeta(cur); dismissConflictBanner(); loadFromText(await cur.text(), fileHandle.name); return; }
            if(choice==='copy'){ fileHandle = await window.showSaveFilePicker({suggestedName:suggestedName(), types:FILE_TYPES}); fileMeta = null; }
            /* 'overwrite' → fall through and write */
          }
        }catch(e){ /* can't read the handle — just proceed with the write */ }
      }
      const w = await fileHandle.createWritable();
      await w.write(data); await w.close();
      try{ recordFileMeta(await fileHandle.getFile()); }catch(e){}   // so our own write doesn't self-trigger
...
async function openClick(){
  ...
  if(window.showOpenFilePicker){
    try{
      ...
      recordFileMeta(f); dismissConflictBanner();
...
function openProject(input){
  ...
  reader.onload = ()=>{ fileHandle=null; fileMeta=null; dismissConflictBanner(); loadFromText(reader.result, file.name); };
...
function applyProject(d){
  ...
  state = {
    ...
    options: d.options.map(o=>({
      ...
      approved: !!o.approved,
      approvedBy: String(o.approvedBy||""), approvedAt: String(o.approvedAt||""),
      revisions: Array.isArray(o.revisions)? o.revisions.slice(-20) : [],
...
function normService(s){
  ...
  return { id:s.id||uid(), qty:numOr(s.qty,0),
    ...
    allowAddons: !!s.allowAddons || (Array.isArray(s.addons) && s.addons.length>0),
...
async function newProject(){
  ...
  fileHandle = null; fileMeta = null; dismissConflictBanner();
  state = blankProject(); dirty=false; resetHistory(); render();
}
window.addEventListener("beforeunload", e=>{ if(dirty){ e.preventDefault(); e.returnValue=""; } });
/* Re-check for an on-disk change whenever the window regains focus. */
window.addEventListener("focus", checkFileConflictOnFocus);
document.addEventListener("visibilitychange", ()=>{ if(!document.hidden) checkFileConflictOnFocus(); });
...
function commandList(){
  ...
  add('Purchase orders by vendor…','Export',()=>openPurchaseOrders());
  ...
  add('Search this job (items, options, schedule)…','Find',()=>openGlobalSearch());
