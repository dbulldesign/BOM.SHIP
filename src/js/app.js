...
function markDirty(){ dirty = true; lastChangeAt = Date.now(); scheduleAutosave(); updateSaveStamp(); }
...
function allowanceFreight(opt){
  ...
  return {
    allowCost:aCost, allowSell:Math.ceil(aCost*fMk),
    freightCost:frCost, freightSell:Math.ceil(frCost*fMk),
    ctrlAllowCost:caCost, ctrlAllowSell:Math.ceil(caCost*cMk)
  };
}
...
function render(){
  ...
  tabs.innerHTML = state.options.map((o,i)=>{
    ...
    return `<button class="tab ${i===state.current?'active':''}" onclick="state.current=${i};render()">
...
function sectionTable(kind, rows, defMarkup, label, tickClass){
  ...
  function fixtureRowHTML(r, i){
    ...
    return `<tr>
      ...
      <td style="width:30px" class="no-print"><button class="rowdel" title="Delete ${allowAccessories?'fixture':'control'}" onclick="delRow('${kind}',${i})">✕</button></td>
    </tr>`;
  }
...
function allowanceFreightRows(kind){
  ...
  function line(label, pctField, pctVal, cost, sell){
    return `<div class="af-line">
      ...
      <span class="af-cost">Cost ${money(cost)}</span>
      <span class="af-sell">Sell <b>${money(sell)}</b></span>
    </div>`;
  }
  if(kind==='fixtures'){
    return `<div class="af-block">
      ${line('Allowance (Fixtures)','allowancePct',opt.allowancePct,af.allowCost,af.allowSell)}
      ${line('Freight (Fixtures)','freightPct',opt.freightPct,af.freightCost,af.freightSell)}
    </div>`;
  }
  if(kind==='controls'){
    return `<div class="af-block">
      ${line('Allowance (Controls)','controlAllowancePct',opt.controlAllowancePct,af.ctrlAllowCost,af.ctrlAllowSell)}
    </div>`;
  }
  return '';
}
  ...
  const groupHTML = groups.map((g,gi)=>{
    const rowsHTML = (g.rows||[]).map((s,si)=>{
      ...
      return `<tr>
        ...
        <td style="width:30px" class="no-print"><button class="rowdel" title="Delete service line" onclick="delService(${gi},${si})">✕</button></td>
      </tr>`;
    }).join('');
...
function renderPane(){
  ...
  const approveBtn = opt.approved
    ? `<span class="approved-badge" title="This option is approved">✓ Approved</span>
       <button class="ghost lock-btn ${opt.baseLocked!==false?'locked':''}" onclick="toggleBaseLock()" title="${opt.baseLocked!==false?'Base is locked — click to unlock for editing':'Base is unlocked — click to lock'}">${opt.baseLocked!==false?'🔒 Locked':'🔓 Unlocked'}</button>
...
function approveOption(){ const o=state.options[state.current]; o.approved = true; if(o.baseLocked===undefined) o.baseLocked = true; markDirty(); render(); toast('Option approved — base locked'); }
function unapproveOption(){ state.options[state.current].approved = false; markDirty(); render(); }
function toggleBaseLock(){ const o=state.options[state.current]; o.baseLocked = (o.baseLocked===false); markDirty(); render(); }
/* Guard: when an approved option's base is locked, confirm before allowing a base edit.
   Returns true if the edit may proceed. */
let _baseUnlockAck = false;   // session ack so we don't nag on every keystroke
function baseEditAllowed(){
  const o = state.options[state.current];
  if(!o.approved || o.baseLocked===false) return true;
  if(_baseUnlockAck) return true;
  return false;
}
...
function renderCompare(){
  ...
  const cards = state.options.map((o,i)=>{
    ...
    return `<div class="cmp-card${isActive?' active':''}${isCheapest&&!isActive?' cheapest':''}" onclick="state.current=${i};render()">
      ${badge}
      ...
      ${savingsTag}
      <button class="cmp-card-goto no-print" onclick="event.stopPropagation();state.current=${i};render()">View option →</button>
    </div>`;
  }).join("");
  ...
  const filterBar = `
    <div class="ship-toolbar">
      ...
      <div class="ship-export-group">
        ...
        <button class="ghost" onclick="printShipSchedule()">PDF / Print</button>
      </div>
    </div>`;
...
function printShipSchedule(){
  ...
  const bodyRows = rows.map(l=>{
    if(l.isCategory){
      return `<tr class="catband"><td colspan="19">${esc2(l.catLabel)} — ${esc2(l.optName)}</td></tr>`;
    }
    if(l.isGroup){
      return `<tr class="grp"><td colspan="19">▸ ${esc2(l.groupName)} <span style="font-weight:normal;color:#777">(${esc2(l.optName)} · ${esc2(l.kind)}s)</span></td></tr>`;
    }
    const m=l.meta||{}; const st=shipStatus(m)[0];
  ...
  const headCells = SHIP_EXPORT_COLS.map(h=>`<th>${h}</th>`).join('');
...
function renderAdvice(){
  ...
  el.innerHTML = head + advices.map(a=>{
    ...
    const itemRows = (a.itemIds||[]).map(rowId=>{
      ...
      if(!l) return `<tr><td colspan="6" class="dim">(item no longer in schedule)</td><td class="no-print"><button class="ship-rowdel" onclick="removeItemFromAdvice('${a.id}','${rowId}')">✕</button></td></tr>`;
    ...
    return `<div class="adv-card">
      ...
      <div class="adv-items">
        ...
        ${(a.itemIds||[]).length? `<table class="adv-items-table">
          <thead><tr><th class="r">Qty</th><th>Cat.</th><th>Type</th><th>Manuf.</th><th>Part #</th><th>Description</th><th class="no-print"></th></tr></thead>
          <tbody>${itemRows}</tbody></table>` : `<div class="adv-noitems">No items yet — add fixtures or controls that haven't shipped.</div>`}
        ${picker}
      </div>
    </div>`;
...
function printEstimate(){
  if(view === 'shipping'){ printShipSchedule(); return; }
...
async function newProject(){
  ...
  fileHandle = null;
  state = blankProject(); dirty=false; render();
}
